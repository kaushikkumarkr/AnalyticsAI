...
from taskweaver.orchestrator.graph import build_graph
...
class AppSessionConfig(ModuleConfig):
    def _configure(self) -> None:
        ...
        self.orchestrator_type = self._get_str("orchestrator_type", "legacy")
...
class Session:
    ...
    def __init__(
        ...
        if self.config.orchestrator_type == "langgraph":
            self.logger.info("Initializing LangGraph orchestrator")
            self.graph_runner = build_graph(self)
    ...
    def _send_text_message(
        ...
        try:
            if self.config.orchestrator_type == "langgraph" and self.graph_runner is not None:
                # LangGraph Execution Path
                self.logger.info("Running via LangGraph Orchestrator")
                
                # Create and add basic User -> Planner post to ensure context exists
                initial_post = Post.create(message=message, send_from="User", send_to="Planner")
                chat_round.add_post(initial_post)
                
                # Create initial state
                # Note: We assume the memory already has the user query as a round/post
                initial_state: TaskWeaverState = {
                    "messages": [initial_post], 
                    "plan": [],
                ...
                # Invoke Graph
                final_state = self.graph_runner.invoke(initial_state)
                
                # Update Chat Round State
                # In Sprint 2, the graph only runs Planner, so we mark round as finished
                chat_round.change_round_state("finished")
                
            elif "planner" in self.config.roles and len(self.worker_instances) > 0:
                # Legacy Logic
